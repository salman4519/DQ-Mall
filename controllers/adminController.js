...
const loadDash = async (req,res)=>{
    ...
    try {
        ...
        res.render('admin/demo',{ categories , orders })
    } catch (error){
...
const cancelOrder = async (req, res) => {
  ...
  const returnOrder = async (req, res) => {
    ...
    try {
        ...
        // Credit the return amount to the wallet
        wallet.Balance += returnAmount; // Increase the wallet balance
        await wallet.save(); // Save the updated wallet

        res.status(200).json({ message: 'Order status updated successfully and amount credited to wallet', order });
    } catch (error) {
        console.error('Error updating order status:', error);
        res.status(500).json({ message: 'Internal server error' });
    }
  ...
  const  getOffer = async (req, res) => {
    ...
    try {
        const products = await Product.find({ Is_list: true });
        const categories = await Category.find({ isList: true })
        const offers = await Offer.find().populate('productIds', 'Name').skip(skip).limit(limit) // Assuming 'Name' is the field for product names
...
const postAddCoupon = async (req, res) => {
    const { couponCode, discountPercentage, expiryDate, minPurchase, maxDiscountAmount } = req.body;
...
const updateCoupon = async (req, res) => {
    try {
        const couponId = req.params.id;
        const { couponCode, discountPercentage, minPurchase, expiryDate, maxDiscountAmount } = req.body;
...
const generateSalesReportPDF = async (req, res) => {
    try {
        ...
        const totalDiscount = orders.reduce((total, order) => total + (order.DiscountedPrice || 0), 0);
        ...
        const reportInfo = reportType === 'custom' 
        ...
        // Table Headers
        const headers = ['Order ID', 'Order Date', 'Payment Method', 'Total Price', 'Discount'];
        const headerWidths = [150, 100, 100, 100, 100]; // Widths for each column

        // Draw headers
        headers.forEach((header, index) => {
            const xPosition = 50 + headerWidths.slice(0, index).reduce((a, b) => a + b, 0);
        ...
        if (orders.length === 0) {
            ...
            orders.forEach((order, index) => {
                ...
                // Draw order details
                page.drawText(`${order.OrderId}`, { x: 50, y: rowYPosition + 2, size: 10, font: font, color: textColor });
                page.drawText(`${orderDate}`, { x: 50 + headerWidths[0], y: rowYPosition + 2, size: 10, font: font, color: textColor });
                page.drawText(`${paymentMethodText}`, { x: 50 + headerWidths[0] + headerWidths[1], y: rowYPosition + 2, size: 10, font: font, color: textColor });
                page.drawText(totalPriceText, { x: 50 + headerWidths[0] + headerWidths[1] + headerWidths[2], y: rowYPosition + 2, size: 10, font: font, color: textColor });
                page.drawText(`$${discount.toFixed(2)}`, { x: 50 + headerWidths[0] + headerWidths[1] + headerWidths[2] + headerWidths[3], y: rowYPosition + 2, size: 10, font: font, color: textColor });
            });
        }
        ...
        summaryData.forEach((item, index) => {
            yPosition -= 20;
            page.drawText(item.label, { x: 50, y: yPosition, size: 12, font: font, color: headerColor });
            page.drawText(item.value, { x: 150, y: yPosition, size: 12, font: font, color: textColor });
        });
        ...
        const reportFilePath = path.join(__dirname, 'reports', `Sales_Report_${reportType}_${moment().format('YYYYMMDD_HHmmss')}.pdf`);
...
const generateSalesReportExcel = async (req, res) => {
    try {
        ...
        console.error("Error generating sales report:", error);
        res.status(500).json({ error: "An error occurred while generating the sales report.", details: error.message });
    }
};
    ...
    switch (reportType) {
        ...
        case 'custom':
            if (!startDate || !endDate) {
                return res.status(400).json({ error: "Both start and end dates are required for custom reports." });
            }
...
const createAndSendExcelReport = async (orders, reportType, res) => {
    ...
    try {
        ...
        orders.forEach(order => {
            ...
            worksheet.addRow({
                OrderId: order.OrderId || '',
                createdAt: order.createdAt ? moment(order.createdAt).format('YYYY-MM-DD') : '',
        ...
        res.status(500).json({ error: "Error creating Excel file.", details: error.message });
    }
};
